...
) : RecyclerView.Adapter<UserProfileAdapter.Holder>() {

    val sessionManager = SessionManager(requireContext)

    class Holder(val binding: ItemUserProfileBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(dataItem: UserData, requireContext: Activity) {
            binding.tvUserName.text = dataItem.name
            binding.tvUserRole.text = dataItem.role
            if (dataItem.avatar == null || dataItem.avatar == "") {
                ...
                Glide.with(requireContext)
                    .load(AppConstant.MEDIA_BASE_URL + dataItem.avatar)
                    .into(binding.tvLogo)
            }
        }
    }
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            ...
            Navigation.findNavController(holder.itemView)
                .navigate(R.id.openUserProfileViewScreen, bundle)
        }
